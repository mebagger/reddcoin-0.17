...
public:
     ...
     */
    bool AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient=false) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS=true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
const std::string strMessageMagic = "Reddcoin Signed Message:\n";
...
static bool WriteBlockToDisk(const CBlock& block, CDiskBlockPos& pos, const CMessageHeader::MessageStartChars& messageStart)
{
    ...
    printf("WriteBlockToDisk() Block: %s \n", block.ToString().c_str() );
    return true;
}
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    ...
    printf("ReadBlockFromDisk() Block: %s \n", block.ToString().c_str() );            
    return true;
}
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(); // PoSV: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // PoSV
            undo.nTime = alternate.nTime;           // PoSV
        } else {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    //}
    // find the v4 hieght on this chain then use (pindex->nHeight >= consensusparams.BIP66Height)
    if (pindex->nVersion >= 4 &&
    	IsSuperMajority(4, pindex->pprev, consensusparams.nRejectBlockOutdatedMajority_4))
    {
    	flags |= SCRIPT_VERIFY_DERSIG;
    }
...
bool ReddcoinContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (!ComputeNextStakeModifier(pindex->pprev, nStakeModifier, fGeneratedStakeModifier))
        return error("ReddcoinContextualBlockChecks() : ComputeNextStakeModifier() failed");
  ...
  // compute nStakeModifierChecksum begin
    ...
    uint256 hashProofOfStakeBackup = pindex->hashProof;

    // set necessary pindex fields
    if (!pindex->SetStakeEntropyBit(nEntropyBit))
        return error("ReddcoinContextualBlockChecks() : SetStakeEntropyBit() failed");
    ...
    pindex->hashProof = hashProofOfStakeBackup;
	...
	if (hash != Params().GetConsensus().hashGenesisBlock ) {
		...
		else if (block.IsProofOfWork())
		{
            ...
            if(!CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams)){
                fValid = false;
				LogPrintf("WARNING: VerifyHashTarget(): check proof-of-work failed for block %s\n", hash.ToString());
            }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    assert(pindex);
    //LogPrintf("ConnectBlock() Block: %s \n", block.ToString().c_str() );
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.CorruptionPossible()) {
            ...
            // problems.
            LogPrintf("ConnectBlock() Corrupt block found: %s state=%s \n", block.GetHash().ToString(), state.ToString() );
            return AbortNode(state, "Corrupt block found indicating potential hardware failure; shutting down");
        }
        LogPrintf("ConnectBlock() Consensus::CheckBlock: %s state=%s \n", block.GetHash().ToString(), state.ToString() );
    ...
    if (fEnforceBIP30) {
        bool fProofOfStake = pindex->nHeight > chainparams.GetConsensus().nLastProofOfWorkHeight;
        unsigned int j = 0;
        for (const auto& tx : block.vtx) {
            ...
            for (size_t o = 0; o < tx->vout.size(); o++) {
                if (view.HaveCoin(COutPoint(tx->GetHash(), o))) {
                    LogPrintf("ConnectBlock() tried to overwrite transaction: block=%s state=%s tx=%s nHeight=%u, PoSV=%d, vts=%u, vtx.size=%u \n", block.GetHash().ToString(), state.ToString(), tx->GetHash().ToString(), pindex->nHeight, fProofOfStake, j, block.vtx.size() );
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            if (!Consensus::CheckTxInputs(tx, state, view, pindex->nHeight, txfee, Params().GetConsensus())) {
                LogPrintf("ConnectBlock() CheckTxInputs: block=%s state=%s tx=%s \n", block.GetHash().ToString(), state.ToString(), tx.GetHash().ToString() );
                return error("%s: Consensus::CheckTxInputs: %s, %s", __func__, tx.GetHash().ToString(), FormatStateMessage(state));
            }
            ...
            if (!MoneyRange(nFees)) {
                LogPrintf("ConnectBlock() bad-txns-accumulated-fee-outofrange: block=%s state=%s tx=%s \n", block.GetHash().ToString(), state.ToString(), tx.GetHash().ToString() );
            ...
            if (!SequenceLocks(tx, nLockTimeFlags, &prevheights, *pindex)) {
                LogPrintf("ConnectBlock() bad-txns-nonfinal: block=%s state=%s tx=%s \n", block.GetHash().ToString(), state.ToString(), tx.GetHash().ToString() );
        ...
        if (nSigOpsCost > MAX_BLOCK_SIGOPS_COST){
            LogPrintf("ConnectBlock() bad-blk-sigops: block=%s state=%s tx=%s \n", block.GetHash().ToString(), state.ToString(), tx.GetHash().ToString() );
        ...
        if (!tx.IsCoinBase())
        {
            ...
            LogPrintf("ConnectBlock() CheckInputsTrue: block=%s state=%s tx=%s vChecks=%s \n", block.GetHash().ToString(), state.ToString(), tx.GetHash().ToString(), vChecks[0].GetScriptError() );
            control.Add(vChecks);
        }
    ...
    LogPrintf("ConnectBlock() Connect transactions: %s state=%s \n", block.GetHash().ToString(), state.ToString() );
        ...
        if (block.vtx[0]->GetValueOut() > blockReward)
        {
            ...
            return state.DoS(100,
                             error("ConnectBlock() : coinbase pays too much (actual=%d vs limit=%d)",
                                   block.vtx[0]->GetValueOut(), (GetBlockSubsidy(pindex->nHeight, Params().GetConsensus()) + nFees)),
    ...
    else if (block.IsProofOfStake())
    {
        ...
        else
        {
            ...
            if (block.nVersion >= 5 && IsSuperMajority(5, pindex->pprev, Params().GetConsensus().nEnforceBlockUpgradeMajority_5)) {
                ...
                if (nDevEndCredit != nCalculatedDevEndCredit && IsSuperMajority(5, pindex->pprev, Params().GetConsensus().nEnforceBlockUpgradeMajority_5)) {
                    return state.DoS(100, error("ConnectBlock() : bad-dev-amount"));
                }
            }
        }
    ...
    LogPrintf("ConnectBlock() pre-control.Wait(): %s state=%s \n", block.GetHash().ToString(), state.ToString() );
    if (!control.Wait())
        return state.DoS(100, error("%s: CheckQueue failed block-validation-failed", __func__), REJECT_INVALID, "block-validation-failed");
    ...
    BOOST_FOREACH(const CTransactionRef& tx, block.vtx) 
    {
        ...
        BOOST_FOREACH (const CTxIn& in, tx->vin) {
        	//LogPrintf("map mapStakeSpent: Insert %s | %u\n", in.prevout.ToString(), pindex->nHeight);
    ...
    for (map<COutPoint, int>::iterator it = mapStakeSpent.begin(); it != mapStakeSpent.end();) {
        if (it->second < pindex->nHeight - Params().GetConsensus().nMaxReorganizationDepth) {
        	//LogPrintf("map mapStakeSpent: Erase %s | %u\n", it->first.ToString(), it->second);
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    if (!pblock) {
        ...
        {
            LogPrintf("ConnectTip() ReadBlockFromDisk: block=%s state=%s \n", pindexNew->GetBlockHash().ToString(), state.ToString() );
            return AbortNode(state, "Failed to read block");
        }
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        if (!rv) {
            if (state.IsInvalid())
            {
                LogPrintf("ConnectTip() state.IsInvalid: block=%s state=%s \n", pindexNew->GetBlockHash().ToString(), state.ToString() );
                InvalidBlockFound(pindexNew, state);
            }
            LogPrintf("ConnectTip() ConnectBlock failed: block=%s state=%s \n", pindexNew->GetBlockHash().ToString(), state.ToString() );
...
bool CChainState::ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (chainActive.Tip() && chainActive.Tip() != pindexFork) {
        if (!DisconnectTip(state, chainparams, &disconnectpool)) {
            ...
            LogPrintf("ActivateBestChainStep() DisconnectTip: chainActive=%s pindexFork=%s state=%s \n", chainActive.Tip()->GetBlockHash().ToString(), pindexFork->GetBlockHash().ToString(), state.ToString() );
        ...
        for (CBlockIndex *pindexConnect : reverse_iterate(vpindexToConnect)) {
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                if (state.IsInvalid()) {
                    ...
                    if (!state.CorruptionPossible()) {
                        LogPrintf("ActivateBestChainStep() InvalidChainFound: block=%s state=%s \n", pindexConnect->GetBlockHash().ToString(), state.ToString() );
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // PoSV: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // PoSV: first coinbase output should be empty if proof-of-stake block
    ...
    if (block.GetBlockTime() > ( (int64_t)block.vtx[0]->nTime == 0 && !(block.vtx[0]->nVersion > POW_TX_VERSION) ? block.GetBlockTime(): (int64_t)block.vtx[0]->nTime )+  MAX_FUTURE_BLOCK_TIME)
...
{
    LOCK(cs_main);
    //return pindexPrev ? IsBTC16BIPsEnabled(pindexPrev->nTime) : false; // pindexPrev == null on genesis block //PoSV ??
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // PoSV ??: Don't reject in case of old clients. Change our assumption instead.
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus()) ||
        ...
        LogPrintf("AcceptBlock(): error 105.5 = %s\n", block.GetHash().ToString().c_str());
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // PoSV: txindex is always enabled
...
printf("LoadGenesisBlock(1) Block: %s \n", chainparams.GenesisBlock().ToString().c_str() );
    try {
        CBlock &block = const_cast<CBlock&>(chainparams.GenesisBlock());
        printf("LoadGenesisBlock(2) Block: %s \n", block.ToString().c_str() );
        CDiskBlockPos blockPos = SaveBlockToDisk(block, 0, chainparams, nullptr);
        printf("LoadGenesisBlock(3) Block: %s \n", block.ToString().c_str() );
        if (blockPos.IsNull())
            return error("%s: writing genesis block to disk failed", __func__);
        CBlockIndex *pindex = AddToBlockIndex(block, false);
        ReceivedBlockTransactions(block, pindex, blockPos, chainparams.GetConsensus());
        printf("LoadGenesisBlock(4) Block: %s \n", block.ToString().c_str() );
    } catch (const std::runtime_error& e) {
...
bool IsDevTx(const CTransaction& tx)
{
    ...
    if (tx.IsCoinStake()) {
        ...
        if (gArgs.GetBoolArg("-debug", false)) {
            LogPrintf("- mkey=%s\n", HexStr(mkey).substr(0, 24) );
            LogPrintf("- pkey=%s\n", HexStr(pkey).substr(0, 24) );
...
CAmount GetProofOfStakeReward(int64_t nCoinAge, const CAmount& nFees)
{
    ...
    if (gArgs.GetBoolArg("-debug", false) && gArgs.GetBoolArg("-printcreation", false))
