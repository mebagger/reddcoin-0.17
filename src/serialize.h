...
enum
{
    ...
    SER_DISK            = (1 << 1),
    SER_GETHASH         = (1 << 2),
    SER_POSMARKER       = (1 << 18),  // PoSV: for sending block headers with PoS marker, to allow headers-first syncronization
};
...
uint64_t ReadCompactSize(Stream& is)
{
    ...
    if (nSizeRet > (uint64_t)MAX_SIZE){
        printf("ReadCompactSize() nSizeRet: %lu \n", nSizeRet );
        printf("ReadCompactSize() MAX_SIZE: %lu \n", (uint64_t)MAX_SIZE);
